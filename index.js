...
${chalk.green('*')} ${chalk.bold.green('Ninit')} v0.1.0                    ${chalk.green('*')}
${chalk.green('************************************')}\n`);
}

const getConfig = async () => JSON.parse(
...
const checkConfig = async () => {
  try {
    ...
    console.log(`Couldn't find a configuration file, please run ${chalk.bgGreen.black('ninit --config')} to configurate ${chalk.bold.green('Ninit')}.`)
    return false;
  }
}
...
const initializeProject = async ({ workspace, customEditorScript }) => {
  ...
  const { main } = JSON.parse(await fs.promises.readFile(path.join(projectPath, 'package.json')));

  fs.writeFile(path.join(projectPath, main), '', {}, () => {
